...
static void				PrvLookupDrawRecordFields (LookupVariablesPtr vars, void* record, Int16 field, RectanglePtr bounds, Char * phoneLabelLetters);
...
void Lookup(AddrLookupParamsType * params)
{
	...
	UInt16 sortByCompany = GetSortByCompany();
	Boolean contacts = CONTACTS;
	...
	if(!contacts)
		...
		dbP = DmOpenDatabase(0, DmFindDatabase(0, CONTACTS_DBNAME), dmModeReadOnly);  
	...
	if(contacts)
	{
		contactsInfoPtr = (P1ContactsAppInfoPtr) AddrDBAppInfoGetPtr(dbP);
		ToolsInitPhoneLabelLetters(contactsInfoPtr, vars.P1ContactsphoneLabelLetters, contacts);
		MemPtrUnlock(contactsInfoPtr);
	}
	else
	{
		appInfoPtr = (AddrAppInfoPtr) AddrDBAppInfoGetPtr(dbP);
		ToolsInitPhoneLabelLetters(appInfoPtr, vars.phoneLabelLetters, contacts);
		MemPtrUnlock(appInfoPtr);
	}		
	vars.sortByCompany = sortByCompany;
 ...
 *
 ***********************************************************************/
void PrvLookupDrawRecordFields(LookupVariablesPtr vars, void* record, Int16 field, RectanglePtr bounds, Char * phoneLabelLetters)
{
	Char * name1;
	Char * name2;
	Char * field1;
	...
	if (vars->params->field1 == addrLookupSortField || vars->params->field2 == addrLookupSortField)
		name1HasPriority = ToolsDetermineRecordName (record, &shortenedFieldWidth, &fieldSeparatorWidth, vars->sortByCompany, &name1, &name1Length, &name1Width, &name2, &name2Length, &name2Width, NULL, NULL, bounds->extent.x, true, vars->contacts);
	...
	else if(vars->contacts)
	{
		// Map from the lookup field requested to the Address's format
		if (P1ContactsIsPhoneLookupField(field1Num))
		{
			field1 = ((P1ContactsDBRecordPtr)record)->fields[field];
		}
		else if (field1Num == addrLookupListPhone)
		{
			field1 = ((P1ContactsDBRecordPtr)record)->fields[P1Contactsphone1 + ((P1ContactsDBRecordPtr)record)->options.phones.displayPhoneForList];
		}
		else
		{
			field1 = ((P1ContactsDBRecordPtr)record)->fields[vars->lookupFieldMap[field1Num]];
		}
	...
	else
	{
		...
		{
			field1 = ((AddrDBRecordPtr)record)->fields[field];
		}
		else if (field1Num == addrLookupListPhone)
		{
			field1 = ((AddrDBRecordPtr)record)->fields[phone1 + ((AddrDBRecordPtr)record)->options.phones.displayPhoneForList];
		}
		else
		{
			field1 = ((AddrDBRecordPtr)record)->fields[vars->lookupFieldMap[field1Num]];
		}
	...
	{
		// Map from the lookup field requested to the Address's format
		if (P1ContactsIsPhoneLookupField(field2Num))
		{
			field2 = ((P1ContactsDBRecordPtr)record)->fields[field];
		}
		else if (field2Num == addrLookupListPhone)
		{
			field2 = ((P1ContactsDBRecordPtr)record)->fields[P1Contactsphone1 + ((P1ContactsDBRecordPtr)record)->options.phones.displayPhoneForList];
		}
		else
		{
			field2 = ((P1ContactsDBRecordPtr)record)->fields[vars->lookupFieldMap[field2Num]];
		}
	...
	else
	{
		...
		{
			field2 = ((AddrDBRecordPtr)record)->fields[field];
		}
		else if (field2Num == addrLookupListPhone)
		{
			field2 = ((AddrDBRecordPtr)record)->fields[phone1 +  ((AddrDBRecordPtr)record)->options.phones.displayPhoneForList];
		}
		else
		{
			field2 =  ((AddrDBRecordPtr)record)->fields[vars->lookupFieldMap[field2Num]];
		}
	 ...
	 */      {
	 ...
	 if (vars->params->field1 == addrLookupSortField)
	 {
		 ToolsDrawRecordName (name1, name1Length, name1Width, name2, name2Length, name2Width,
						 ...
						 name1HasPriority || !vars->sortByCompany, false, true);
	 ...
	 else if (vars->params->field1 == addrLookupListPhone)
	 {
		 ...
		 if(vars->contacts)
		 {
		 	phoneLabel = GetPhoneLabel((AddrDBRecordPtr)record, firstPhoneField +
										((AddrDBRecordPtr)record)->options.phones.displayPhoneForList);
		 }
		 else
		 {
		 	phoneLabel = GetPhoneLabel((AddrDBRecordPtr)record, firstPhoneField +
	 ...
	 if (vars->params->field2 == addrLookupSortField)
	 {
		 ToolsDrawRecordName (name1, name1Length, name1Width, name2, name2Length, name2Width,
						 horizontalSpace, &x, y, shortenedFieldWidth, fieldSeparatorWidth, false,
						 name1HasPriority || !vars->sortByCompany, false, false);
	 }
	 else if (vars->params->field2 == addrLookupListPhone)
	 {
		 ...
		 if(vars->contacts)
		 {
		 	phoneLabel = P1ContactsGetPhoneLabel((P1ContactsDBRecordPtr)record, P1ContactsfirstPhoneField +
										((P1ContactsDBRecordPtr)record)->options.phones.displayPhoneForList);
		 }
		 else
		 {
		 	phoneLabel = GetPhoneLabel((AddrDBRecordPtr)record, firstPhoneField +
...
void PrvLookupViewDrawRecord (void * table, Int16 row, Int16 column, RectanglePtr bounds)
{
	...
	if(vars->contacts)
	{
		error = PrvP1ContactsDBGetRecord (vars->dbP, recordNum, &contact, &recordH);
	...
	if(vars->contacts)
		PrvLookupDrawRecordFields (vars, &contact, fieldNum, bounds, vars->P1ContactsphoneLabelLetters);
	else
